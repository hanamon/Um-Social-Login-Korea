...
<div class="um-admin-metabox">
	...
	<?php UM()->admin_forms( array(
		...
		'fields' 	=> array(
			...
			array(
				'id'		=> '_um_integration_type',
				'type'		=> 'select',
				'label'    	=> __( 'Integration Type','um-social-login' ),
				'value' 	=> UM()->query()->get_meta_value( '_um_integration_type', null, 'login_register' ),
				'options' 	=> array(
					'login_register'	=> __("Login & Register"),
					//'link_account' 		=> __("Link Account only( For logged-in users )"),
					'login' 			=> __("Login only"),
					'register' 			=> __("Register only"),
				),
			),
		)
	) )->render_form(); ?>
	...
	<?php UM()->admin_forms( array(
		...
		'fields' 	=> array(
			array(
				...
				'size'		=> 'small',
				'placeholder' => '600px',
				'label'    	=> __( 'Container max width', 'um-social-login' ),
